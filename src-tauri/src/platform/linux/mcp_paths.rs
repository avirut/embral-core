...
//! is simply absent, and reports the same thing here.
//!
//! Note what is *not* in this file: `~/.claude.json` (Claude Code) and the
//! codex config path are both plain `$HOME` paths resolved above the seam,
//! so those two clients register on Linux exactly as they do elsewhere.
